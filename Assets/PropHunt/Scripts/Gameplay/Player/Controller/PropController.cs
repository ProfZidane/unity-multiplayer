...
public class PropController : ClassController
{
    ...
    [SerializeField] protected float _tautCoolDownTime = 2.0f;
    private float _nextTautAvailableTime;
    ...
    // Callback when NetworkVariable changed
    ...
    public void SendTauntServerRpc(int tauntNumber)
    {
        if (_isTauntAvailable)
        {
            ...
            _nextTautAvailableTime = Time.time + _tautCoolDownTime;
    ...
    {
        if (IsServer && Time.time >= _nextTautAvailableTime)
        {
